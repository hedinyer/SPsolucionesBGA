-- La vista `atrasos` ignoraba los congelamientos: `periodos_debidos` y
-- `dias_atraso` se calculaban por días calendario desde `fecha_inicio`, sin
-- descontar los días congelados. Como `congelar_cuotas` corre permanentemente
-- las fechas de vencimiento (+p_dias, acumulativo), un cliente congelado y al
-- día seguía apareciendo en mora.
--
-- Fix: restar el total de días congelados de la compra al calcular el tiempo
-- transcurrido (periodos_debidos) y correr el ancla de `dias_atraso`.
