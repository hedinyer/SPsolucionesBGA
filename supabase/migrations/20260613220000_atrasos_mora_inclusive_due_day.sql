-- Mora cuenta desde el día de vencimiento (inclusive), no al día siguiente
...
-- morosos: permitir registrar desde el día 1 de atraso
ALTER TABLE public.morosos DROP CONSTRAINT IF EXISTS morosos_dias_atraso_check;
ALTER TABLE public.morosos
  ADD CONSTRAINT morosos_dias_atraso_check CHECK (dias_atraso >= 1);
...
BEGIN
  ...
  -- Vence hoy = vencida (no esperar al día siguiente)
