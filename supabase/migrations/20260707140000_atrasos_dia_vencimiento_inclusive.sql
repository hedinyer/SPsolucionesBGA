-- Restaura el conteo inclusive del día de vencimiento en `dias_atraso`.
-- La migración 20260613220000 ("mora inclusive") ya había establecido que el
-- día de vencimiento cuenta como día 1 de mora (dias_atraso = ... + 1), pero al
-- reescribir la vista en 20260621110000 (excluir 'inicial') se perdió ese +1,
-- dejando el conteo exclusivo. Resultado: un cliente diario que debía 3 cuotas
-- (periodos 2,3,4 vencidos) mostraba 2 días de atraso en vez de 3.
--
-- Esta vista conserva además: exclusión de inicial/visita/cuota_adelantada y el
-- respeto a los congelamientos.
...
CROSS JOIN LATERAL (
  SELECT
    ...
    CASE
      ...
      ELSE base.fecha_inicio + frz.dias_congelados + (
        GREATEST(
          0,
          FLOOR(
    ...
    CASE
      ...
      -- Inclusive: el día de vencimiento ya es día 1 de mora
      ELSE (tz.v_today - (
